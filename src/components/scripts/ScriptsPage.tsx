...
import { Plus, Copy, Edit, Trash2, FileDown, MessageCircle, FileText } from 'lucide-react';
...
async function gerarDocx(campos: { nome: string; empresa: string; data: string; valor: string }): Promise<Blob> {
  const JSZipModule = await import('jszip');
  const JSZip = JSZipModule.default;
  const zip = await JSZip.loadAsync(base64ToArrayBuffer(PROPOSTA_BASE64));
...
function PropostaGenerator() {
  ...
  const handleGerar = async () => {
    ...
    setLoading(true);
    try {
      const blob = await gerarDocx(form);
  ...
  const handleWhatsApp = () => {
    ...
    const texto = "Olá " + form.nome + "! 😊\n\n"
      + "Segue a proposta da Click Fácil" + empresa + ".\n\n"
  ...
  return (
    <Card className="border-primary/30">
      <CardHeader>
        ...
        <CardDescription>Preencha os campos e baixe o .docx pronto para enviar</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button onClick={handleWhatsApp} disabled={!pronto} variant="outline" className="flex-1 gap-2 border-green-500 text-green-600 hover:bg-green-50" size="lg">
...
export function ScriptsPage({ scripts, onAddScript, onUpdateScript, onDeleteScript }: ScriptsPageProps) {
  ...
  const handleAddFromTemplate = (template: typeof TEMPLATES_PRONTOS[0]) => {
    ...
    toast({ title: "Template adicionado!", description: `"${template.title}" foi salvo.` });
  };

  const handleCopy = (content: string) => {
    navigator.clipboard.writeText(content);
    toast({ title: "Copiado!", description: "Script copiado." });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Roteiros de Mensagens</h1>
          <p className="text-muted-foreground mt-1">Templates prontos para agilizar seu contato com leads</p>
        </div>
        <Button onClick={handleNew} className="gap-2"><Plus className="w-4 h-4" />Novo Roteiro</Button>
      </div>
          ...
          {TEMPLATES_PRONTOS.map((template, index) => (
            <Card key={index}>
              ...
              <CardContent className="space-y-3">
                <div className="bg-muted p-3 rounded text-sm whitespace-pre-wrap max-h-40 overflow-y-auto">{template.content}</div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => handleCopy(template.content)} className="gap-2 flex-1"><Copy className="w-4 h-4" />Copiar</Button>
                  <Button size="sm" onClick={() => handleAddFromTemplate(template)} className="gap-2 flex-1"><Plus className="w-4 h-4" />Salvar</Button>
                </div>
              </CardContent>
            </Card>
          ))}
                ...
                <CardHeader>
                  <CardTitle className="text-base flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      <Button size="sm" variant="ghost" onClick={() => handleEdit(script)}><Edit className="w-4 h-4" /></Button>
                      <Button size="sm" variant="ghost" onClick={() => onDeleteScript(script.id)}><Trash2 className="w-4 h-4 text-destructive" /></Button>
                    </div>
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="bg-muted p-3 rounded text-sm whitespace-pre-wrap max-h-40 overflow-y-auto">{script.content}</div>
                  <Button size="sm" variant="outline" onClick={() => handleCopy(script.content)} className="gap-2 w-full"><Copy className="w-4 h-4" />Copiar</Button>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Dica: Use [NOME DA EMPRESA], [NOME], [NICHO] como placeholders</p>
            </div>
